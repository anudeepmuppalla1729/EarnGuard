...
const RISK_THRESHOLD = 0.5; // Threshold to trigger claim payouts

/** Weather logic computation */
function computeWeatherRisk(weatherData: any): number {
  let score = 0.0;
  if (
    weatherData?.condition === "CYCLONE" ||
...
export const disruptionWorker = new Worker(
  DISRUPTION_QUEUE_NAME,
  async (job: Job) => {
    console.log(
      `[DisruptionWorker] Starting 3-minute interval detection... JobID: ${job.id}`,
    );
    ...
    try {
      ...
      for (const cityId of cities) {
        ...
        cityNewsCache[cityId] = Math.max(
          prologRisk,
          (newsData?.confidence || 0.0) * 0.5,
        );
      ...
      for (const zoneId of zones) {
        ...
        console.log(
          `[DisruptionWorker] Zone: ${zoneId} | Final Computed Risk: ${finalRisk.toFixed(2)} | Order Drops: ${zoneOrderDrops[zoneId]}%`,
        );
      }
