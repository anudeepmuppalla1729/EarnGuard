...
export const payoutWorker = new Worker(
  ...
  async (job: Job) => {
    console.log(
      `[PayoutWorker] Starting hourly aggregation and payout execution... JobID: ${job.id}`,
    );
    ...
    try {
      ...
      // Step 2: Aggregate 3-min snapshots over the last 10 minutes (Demo Mode)
      ...
      if (aggregatedRes.rows.length === 0) {
        console.log(
          "[PayoutWorker] No risk snapshots found in the last hour. Nothing to calculate.",
        );
        await client.query("COMMIT");
        return;
      }
      ...
      for (const row of aggregatedRes.rows) {
        ...
        console.log(
          `[PayoutWorker] Zone: ${zoneId} | Hourly Avg Risk: ${avgRisk.toFixed(2)} | Max Order Drops: ${maxDrops}%`,
        );
      }
      ...
      for (const zoneId of zonesToProcess) {
        ...
        const onlineRes = await fetch(
          `${SIM_URL}/platform/active-workers?zoneId=${zoneId}`,
          {
            method: "POST",
        ...
        const onlineData = onlineRes.ok
          ...
          : { onlineWorkerIds: [] };
        onlineData.onlineWorkerIds.forEach((id: string) => {
          onlineWorkersMap[id] = true;
        });
      }
      ...
      recentClaimsRes.rows.forEach((r) => {
        duplicateCooldownMap[r.worker_id] = true;
      });
      ...
      for (const worker of activeWorkers) {
        ...
        if (!onlineWorkersMap[worker.platform_worker_id]) {
          console.log(
            `[PayoutWorker] Skipping Worker ${worker.worker_id} - Offline/Inactive.`,
          );
          continue;
        }
        ...
        if (duplicateCooldownMap[worker.worker_id]) {
          console.log(
            `[PayoutWorker] Skipping Worker ${worker.worker_id} - CD Active.`,
          );
          continue;
        }
        ...
        const statRes = await fetch(
          `${SIM_URL}/platform/workers/${worker.platform_worker_id}/income-stats?hour=${currentHour}`,
        );
        ...
        const baseCoverage =
          parseFloat(worker.coverage_multiplier) *
          intervalLoss;
            ...
            await client.query(
              ...
              [
                ...
                JSON.stringify({
                  ...
                  amount: payout,
                  reason: "Aggregated Hourly Risk Exceeded",
                }),
              ],
            );

            console.log(
              `[PayoutWorker] => Paid out ₹${payout.toFixed(2)} to Worker ${worker.worker_id}`,
            );
          } else {
            console.log(
              `[PayoutWorker] => Queued PENDING manual claim for Worker ${worker.worker_id}`,
            );
          }
        }
      }
      ...
      await client.query(
        `DELETE FROM zone_risk_snapshots WHERE created_at < NOW() - INTERVAL '24 hours'`,
      );

      await client.query("COMMIT");
      console.log(
        `[PayoutWorker] 1-Hour Aggregation & Payout Routine complete.`,
      );
    } catch (err: any) {
